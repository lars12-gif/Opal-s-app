...
# 2. رابط الصوت المباشر من مستودعك على GitHub
GITHUB_AUDIO_URL = (
    "https://raw.githubusercontent.com/lars12-gif/Opal-s-app/main/bgm.mp3"
)
...
# 4. تشغيل الموسيقى التلقائية من GitHub + البارتكلز والأصوات
components.html(
    f"""
    <audio id="bgm-audio" loop src="{GITHUB_AUDIO_URL}"></audio>
        ...
        if not df_m.empty:
            ...
            with col_a:
                ...
                if st.button("✏️ تحديث الرصيد"):
                    conn = get_connection()
                    c = conn.cursor()
                    sql_update = "UPDATE members SET oplz = ? WHERE name = ?"
                    c.execute(sql_update, (new_val, selected))
                    conn.commit()
                    conn.close()
            ...
            with col_b:
                if st.button(f"❌ حذف {selected}"):
                    conn = get_connection()
                    c = conn.cursor()
                    sql_delete = "DELETE FROM members WHERE name = ?"
                    c.execute(sql_delete, (selected,))
                    conn.commit()
                    conn.close()
                    st.rerun()
        else:
