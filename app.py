...
# ---------------------------------------------------------
# 2. إخفاء الهيدر وأزرار GitHub والعلامات المائية (CSS)
# ---------------------------------------------------------
st.markdown("""
<style>
    /* إخفاء الشريط العلوي بالكامل وأزرار GitHub والـ Deploy */
    ...
    /* إخفاء العلامة المائية والحقوق بالأسفل */
    footer, 
    ...
    .ruby-btn {
        ...
        margin-top: 6px;
        box-shadow: 0 3px 10px rgba(136, 14, 79, 0.3);
...
def get_all_opals():
    if not supabase:
        return pd.DataFrame(columns=["name", "oplz", "role"])
    try:
        res = supabase.table("opals_members").select("*").order("oplz", desc=True).execute()
...
def save_or_update_member(name, added_oplz, role):
    if not supabase:
        return False
    try:
        res = supabase.table("opals_members").select("*").eq("name", name).execute()
        if res.data:
            current_oplz = float(res.data[0].get("oplz", 0))
            new_total = current_oplz + float(added_oplz)
            supabase.table("opals_members").update({"oplz": new_total, "role": role}).eq("name", name).execute()
        else:
            supabase.table("opals_members").insert({"name": name, "oplz": float(added_oplz), "role": role}).execute()
        return True
    except Exception:
        return False

def set_member_points(name, new_oplz):
    if supabase:
        try:
            supabase.table("opals_members").update({"oplz": float(new_oplz)}).eq("name", name).execute()
            return True
        except Exception:
            return False
    return False
...
def delete_member(name):
    if supabase:
        try:
            supabase.table("opals_members").delete().eq("name", name).execute()
            return True
        except Exception:
            return False
    return False
...
def create_html_card(df):
    ...
    for rank, row in enumerate(df.itertuples(index=False), start=1):
        name, oplz, role = str(row.name), float(row.oplz), str(row.role)
    ...
    <head>
      ...
      <style>
        * {{ box-sizing: border-box; font-family: 'Cairo', sans-serif; margin: 0; padding: 0; }}
        ...
        .dl-btn {{ background: linear-gradient(90deg, #EC407A 0%, #D81B60 100%); color: #FFFFFF; font-weight: 900; font-size: 16px; border: none; padding: 14px 28px; border-radius: 16px; cursor: pointer; box-shadow: 0 4px 18px rgba(216, 27, 96, 0.35); width: 100%; max-width: 420px; }}
        @media (max-width: 600px) {{ .grid-container {{ grid-template-columns: 1fr; }} .header {{ flex-direction: column; gap: 10px; }} }}
      </style>
    </head>
    <body>
...
# ---------------------------------------------------------
# 6. الواجهة الرئيسية وبنك الروبي
# ---------------------------------------------------------
...
with tab1:
    ...
    if not df.empty:
        ...
        st.info("💡 لا يوجد أعضاء مسجلين حتى الآن في قاعدة البيانات السحابية.")
...
with tab2:
    ...
    if pwd_tab2 == ADMIN_PASSWORD:
        st.success("🔓 تم التحقق بنجاح! يمكنك الآن تسجيل النقاط بالسحابة.")
        ...
        with st.form("add_form", clear_on_submit=True):
            ...
            if st.form_submit_button("🚀 حفظ وزيادة النقاط"):
                if name_input.strip():
                    ...
                    if save_or_update_member(clean_name, val_input, clean_role):
                        st.success(f"✅ تم إضافة {val_input:g} Opal's لـ {clean_name} وحفظها بالسحابة!")
                        st.rerun()
                    else:
                        st.error("حدث خطأ أثناء الحفظ بقاعدة البيانات السحابية.")
    else:
        if pwd_tab2:
...
with tab3:
    ...
    if pwd_tab3 == ADMIN_PASSWORD:
        ...
        if not df_m.empty:
            selected = st.selectbox("اختر عضواً للتعديل أو الحذف:", df_m["name"].tolist())
            ...
            with col_a:
                ...
                if st.button("✏️ تحديث الرصيد"):
                    if set_member_points(selected, new_val):
                        st.success("تم التحديث بالسحابة!")
                        st.rerun()
                    else:
                        st.error("حدث خطأ في التحديث.")

            with col_b:
                if st.button(f"❌ حذف {selected}"):
                    if delete_member(selected):
                        st.success(f"تم حذف {selected} بنجاح!")
                        st.rerun()
                    else:
